...
class UnitExport implements FromCollection,WithHeadings,WithMapping,WithEvents,WithCustomStartCell
{
    /**
    ...
    */
    ...
    public function __construct($key_name, $sortingName, $search, $searchName, $searchAddress){
        $this->key_name = trim($key_name);
        $this->sortingName = trim($sortingName);
        $this->search = trim($search);
        $this->searchName= trim($searchName);
        $this->searchAddress= trim($searchAddress);
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $event->sheet->getDelegate()->setCellValue("A1", "Table Unit data");
