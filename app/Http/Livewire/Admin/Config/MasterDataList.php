...
class MasterDataList extends BaseLive
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.config.master.master-data-list', ['category' => $data, 'dataType' => $dataType]);
    }
    ...
    public function store()
    {
        ...
        $master->image =$this->image ? $this->image->store('uploads/imagess') : null;
        $master->save();
    ...
    {
        $masterData = MasterData::findOrFail($this->deleteId);
        if($masterData->image&&file_exists('./storage/'. $masterData->image)){
            unlink('./storage/'. $masterData->image);
        }
        $masterData->delete();
    ...
    public function update(){
        ...
        if(isset($this->remove_path) && $this->remove_path){
            if(file_exists('./storage/'. $this->remove_path)){
                unlink('./storage/'. $this->remove_path);
            }
            $master->image = null;
        }
        if ($this->change_image && $this->image) {
            $master->image =$this->image->store('uploads/images');
        }
        $master->save();
        $this->resetform();
    ...
    public function setNoteCreate($note, $note_en) {
        $this->note = $note;
        $this->note_en = $note_en;
        $this->store();
    }

    public function setNoteUpdate($note, $note_en) {
        $this->note = $note;
        $this->note_en = $note_en;
        $this->update();
    }
